...
function ProjectBoard({ projectId, onBack }: { projectId: string; onBack: () => void }) {
  ...
  return <main className="project-board min-h-screen bg-slate-950 p-4 text-slate-100 md:p-8"><div className="mx-auto max-w-7xl">
    ...
    <section className="project-task-board space-y-4"><div className="flex items-center justify-between"><div><h2 className="text-sm font-bold">{WORKSPACE_MODULES.find((module) => module.id === activeModule)?.name}</h2><p className="mt-1 text-xs text-slate-400">拖曳任務卡到其他欄位，或使用卡片內的狀態選單調整流程。</p></div></div>
