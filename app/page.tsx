...
import { ExpenseCategory, EXPENSE_CATEGORIES, EXPENSE_CATEGORY_LABELS, ModuleId, PROJECT_TYPES, ProjectListItem, ProjectType, WORKSPACE_MODULES } from '@/types/project';
...
function ProjectBoard({ projectId, onBack }: { projectId: string; onBack: () => void }) {
  ...
  return <main className="project-board min-h-screen bg-slate-950 p-4 text-slate-100 md:p-8"><div className="mx-auto max-w-7xl">
    ...
    <div className="grid gap-6 lg:grid-cols-4"><nav className="h-fit rounded-xl border border-slate-800 bg-slate-900/40 p-4">{WORKSPACE_MODULES.map((module) => <button key={module.id} onClick={() => { setActiveModule(module.id); setAddingStatus(null); }} className={`mb-1 w-full rounded-lg px-4 py-3 text-left text-sm font-semibold ${activeModule === module.id ? 'bg-indigo-600' : 'text-slate-400 hover:bg-slate-800'}`}>{module.name}</button>)}</nav>
      <section className="space-y-4 lg:col-span-3"><div className="flex items-center justify-between"><div><h2 className="text-sm font-bold">{WORKSPACE_MODULES.find((module) => module.id === activeModule)?.name}</h2><p className="mt-1 text-xs text-slate-400">拖曳任務卡到其他欄位，或使用卡片內的狀態選單調整流程。</p></div></div>
