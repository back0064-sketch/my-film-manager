...
import { MonthlySettlement, ProjectData, SettlementBatch, Task, TransactionType } from '@/types/project';
...
export function ProjectFinancePanel({ project, updateBudget, updateTask, updateSettlementBatch, deleteTask, addTask, onBackToTasks }: Props) {
  ...
  return <main className="finance-panel min-h-screen bg-slate-950 p-4 pb-24 text-slate-100 md:p-8 md:pb-8"><div className="mx-auto max-w-7xl">
    ...
    {project.settlementBatches.length > 0 && <section className="mt-6 rounded-2xl border border-amber-800/70 bg-slate-900/60 p-4 sm:p-5"><h2 className="font-bold">專案月結追款</h2><p className="mt-1 text-xs text-slate-400">每個月份獨立請款，不與同一客戶的其他專案合併。</p><div className="mt-4 grid gap-3 lg:grid-cols-2">{[...project.settlementBatches].sort((a, b) => b.month.localeCompare(a.month)).map((batch) => <article key={batch.id} className="rounded-xl border border-slate-800 bg-slate-950/60 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-bold">{batch.month} · {batch.items.length} 支</p><p className="mt-1 text-lg font-black text-emerald-400">{money(batch.total)}</p>{batch.invoiceNumber && <p className="mt-1 text-xs text-slate-400">請款編號：{batch.invoiceNumber}</p>}{batch.dueDate && batch.status !== 'paid' && <p className="mt-1 text-xs text-amber-300">付款期限：{batch.dueDate}</p>}</div><select value={batch.status} onChange={(event) => updateSettlementBatch(batch.id, { status: event.target.value as SettlementBatch['status'] })} className="rounded-lg border border-slate-700 bg-slate-950 px-2 py-2 text-xs"><option value="draft">待請款</option><option value="invoiced">已請款</option><option value="paid">已收款</option></select></div><label className="mt-4 block text-xs text-slate-400">請款編號<input defaultValue={batch.invoiceNumber ?? ''} onBlur={(event) => updateSettlementBatch(batch.id, { invoiceNumber: event.target.value.trim() || undefined })} className="mt-1 w-full rounded border border-slate-700 bg-slate-950 px-2 py-2 text-slate-100" /></label></article>)}</div></section>}
