...
function BudgetPanel({ project, expenses, updateBudget }: { project: ProjectData; expenses: Task[]; updateBudget: Props['updateBudget'] }) { return <section className="mt-6 rounded-2xl border border-slate-800 bg-slate-900/60 p-4 sm:p-5"><h2 className="font-bold">成本控制（選用）</h2><div className="mt-4 grid gap-3 md:grid-cols-3">{EXPENSE_CATEGORIES.map((category) => { const actual = expenses.filter((task) => task.expenseCategory === category && task.isPaid).reduce((sum, task) => sum + task.amount, 0); return <div key={category} className="rounded-xl bg-slate-950/60 p-4"><label className="text-xs text-slate-400">{EXPENSE_CATEGORY_LABELS[category]}預算<input type="number" min="0" defaultValue={project.budgetByCategory[category]} onBlur={(event) => updateBudget(category, Math.max(0, Number(event.target.value) || 0))} className="mt-2 w-full rounded border border-slate-700 bg-slate-950 px-2 py-2" /></label><p className="mt-3 text-xs">實際 {money(actual)}</p></div>; })}</div></section>; }
